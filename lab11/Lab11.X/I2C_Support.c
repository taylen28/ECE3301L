...
{
    char Device = 0x48;      // DS1621 device address
    char Cmd = READ_TEMP;    // Command to read temperature
    char Data_Ret;    
...
{
    char Device = 0x68;      // DS3231 device address
    char Address = 0x00;     // Start address to read time
...
DS1621_Read_Temp_Bad()
{
    char Device = 0x49;      // DS1621 device address
    char Cmd = READ_TEMP;    // Command to read temperature
    char Data_Ret;    
    I2C_Start();                      // Start I2C protocol
    I2C_Write((Device << 1) | 0);     // Device address
